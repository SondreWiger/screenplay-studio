...
export function canAccess(
  ...
  if (flagKey === 'pro_subscription') return true;
...
export function useFeatureFlags() {
  ...
  const hasAccess = useCallback(
    (featureKey: string): boolean => {
      ...
      return canAccess(flag.tier, user?.insider_tier ?? null, user?.is_pro ?? false, flag.key);
    },
    [flags, user?.insider_tier, user?.is_pro]
  );
  ...
  return { flags, loading, hasAccess, getFlag, getTier, refresh, userTier: user?.insider_tier ?? null, isPro: user?.is_pro ?? false };
}
