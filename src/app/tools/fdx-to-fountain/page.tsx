...
import { generateFountain } from '@/lib/scripts/fountain';
...
export default function FdxToFountainPage() {
  ...
  const handleFile = useCallback((f: File) => {
    if (!f.name.toLowerCase().endsWith('.fdx')) { setError('Select an FDX file.'); return; }
  ...
  const convert = useCallback(async () => {
    ...
    try {
      ...
      a.download = file.name.replace(/\.fdx$/i, '.fountain'); a.click();
      URL.revokeObjectURL(url); setStatus('done');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to convert FDX'); setStatus('error');
    }
  }, [file]);
  ...
  return (
    <ConverterLayout title="FDX → Fountain" description="Convert a Final Draft XML file to Fountain plain text.">
      <div className="flex flex-col items-center gap-8 max-w-lg mx-auto">
        <UploadZone onFile={handleFile} accept=".fdx" label="Drop an FDX file here or click to browse" sublabel="Final Draft XML (.fdx)">
          {file ? (
            <div>
