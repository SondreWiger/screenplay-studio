    ...
    try {
      ...
      for (const el of elements) {
        if (el.element_type === 'scene_heading') {
          ...
          cur = {
            number: el.scene_number || String(n),
            heading: h,
            location: tM ? loc.replace(/\s*[-–]\s*$/, '').trim() : loc,
            timeOfDay: tM ? tM[1] : '',
            wordCount: h.split(/\s+/).length,
            pageEstimate: '',
          };
        } else if (cur) {
          cur.wordCount += (el.content?.split(/\s+/).length || 0);
        }
      }
      if (cur) list.push(cur);
      for (const s of list) {
        const p = s.wordCount / 250;
        s.pageEstimate = p < 0.1 ? '< 0.1' : p.toFixed(1);
      }
      setScenes(list);
    } catch (e) {
          ...
          <div className="w-full max-w-3xl space-y-6">
            ...
            <div style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              <table className="w-full text-xs">
                ...
                <tbody>
                  {scenes.map(s => (
                    <tr key={s.number} className="hover:bg-white/[0.02] transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
