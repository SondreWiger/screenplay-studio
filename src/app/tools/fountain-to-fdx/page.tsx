...
export default function FountainToFdxPage() {
  ...
  const handleFile = useCallback((f: File) => {
    ...
    if (!ext.endsWith('.fountain') && !ext.endsWith('.txt')) { setError('Select a Fountain or .txt file.'); return; }
  ...
  const convert = useCallback(async () => {
    ...
    try {
      const result = parseFountain(await file.text());
      ...
      a.download = file.name.replace(/\.(fountain|txt)$/i, '.fdx'); a.click();
      URL.revokeObjectURL(url); setStatus('done');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Failed to convert Fountain'); setStatus('error');
    }
  }, [file]);
  ...
  return (
    <ConverterLayout title="Fountain → FDX" description="Convert a Fountain plain text screenplay to Final Draft XML.">
      <div className="flex flex-col items-center gap-8 max-w-lg mx-auto">
        <UploadZone onFile={handleFile} accept=".fountain,.txt" label="Drop a Fountain file here or click to browse" sublabel="Fountain (.fountain) or plain text (.txt)">
          {file ? (
            <div>
