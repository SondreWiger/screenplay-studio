...
export default function MoodBoardPage({ params }: { params: { id: string } }) {
  ...
  const renderItemCard = (item: MoodBoardItem, isCanvas: boolean) => {
    ...
    const baseClass = cn(
      ...
      isCanvas && isConnectTarget && 'hover:ring-2 hover:ring-brand-500',
    );
          ...
          <div className="flex items-center gap-1 ml-2">
            <button
              ...
              className={cn(
                ...
                activeSection === 'all' ? 'bg-brand-500/20 text-brand-500' : 'text-surface-400 hover:text-white hover:bg-surface-900/5'
              )}
            >
              All
            </button>
            {BOARD_SECTIONS.map((section) => (
              <button
                ...
                className={cn(
                  'px-2.5 py-1 rounded-full text-xs font-medium transition-colors whitespace-nowrap',
                  activeSection === section.value ? 'bg-brand-500/20 text-brand-500' : 'text-surface-400 hover:text-white hover:bg-surface-900/5'
                )}
              >
        ...
        >
          ...
          {canvasTool === 'connect' && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-surface-900 border border-surface-700 rounded-lg px-4 py-2 text-sm text-surface-300 shadow-xl z-[10001]">
              ...
              <button onClick={() => { setCanvasTool('move'); setConnectingFrom(null); }} className="ml-3 text-brand-500 hover:text-brand-400 text-xs font-medium">Cancel</button>
            </div>
          )}
                  ...
                  {([{ value: 'solid', label: 'Solid' }, { value: 'dashed', label: 'Dashed' }, { value: 'dotted', label: 'Dotted' }] as const).map((s) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedConnection.line_style === s.value ? 'bg-brand-500/20 text-brand-500' : 'bg-surface-800 text-surface-400 hover:text-white'
                      )}
                    >
                      {s.label}
                    </button>
              ...
              {([
                ...
                <button
                  ...
                  className={cn(
                    ...
                    newItemType === t.value ? 'bg-brand-500/20 text-brand-500 ring-1 ring-brand-500/30' : 'bg-surface-800 text-surface-400 hover:text-white'
                  )}
                >
              ...
              {BOARD_SECTIONS.map((s) => (
                <button
                  ...
                  className={cn(
                    ...
                    newItemSection === s.value ? 'bg-brand-500/20 text-brand-500' : 'bg-surface-800 text-surface-400 hover:text-white'
                  )}
                >
                  {s.icon} {s.label}
                </button>
            ...
            <div className="flex flex-wrap gap-1.5">
              {BOARD_SECTIONS.map((s) => (
                <button
                  ...
                  className={cn(
                    'px-2.5 py-1 text-xs rounded-full transition-colors',
                    editItem.board_section === s.value ? 'bg-brand-500/20 text-brand-500' : 'bg-surface-800 text-surface-400 hover:text-white'
                  )}
                >
                  {s.icon} {s.label}
                </button>
