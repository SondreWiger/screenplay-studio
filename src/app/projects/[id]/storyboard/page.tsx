          ...
          {editorTab === 'draw' && (
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                <button onClick={() => setDrawTool('pen')} className={cn('p-1.5 sm:p-2 rounded-lg transition-colors', drawTool === 'pen' ? 'bg-brand-600/20 text-brand-500' : 'text-surface-400 hover:text-white hover:bg-surface-900/5')} title="Pen">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </button>
                <button onClick={() => setDrawTool('eraser')} className={cn('p-1.5 sm:p-2 rounded-lg transition-colors', drawTool === 'eraser' ? 'bg-brand-600/20 text-brand-500' : 'text-surface-400 hover:text-white hover:bg-surface-900/5')} title="Eraser">
