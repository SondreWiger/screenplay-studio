      ...
      <div className="border-b border-surface-800 bg-surface-950/80 backdrop-blur-sm px-4 py-2 flex items-center justify-between gap-2 z-10">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-0.5 ml-2">
            <button onClick={zoomIn} className="p-1.5 rounded-lg text-surface-400 hover:text-white hover:bg-white/5">
            ...
            <button onClick={zoomOut} className="p-1.5 rounded-lg text-surface-400 hover:text-white hover:bg-white/5">
            ...
            <button onClick={fitToContent} className="p-1.5 rounded-lg text-surface-400 hover:text-white hover:bg-white/5" title="Fit to content">
      ...
      >
        ...
        {tool !== 'select' && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-surface-900 border border-surface-700 rounded-lg px-4 py-2 text-sm text-surface-300 shadow-xl z-20">
            ...
            <button onClick={() => { setTool('select'); setConnectingFrom(null); }} className="ml-3 text-brand-400 hover:text-brand-300 text-xs font-medium">Cancel (Esc)</button>
          </div>
        )}
      </div>
        ...
        <div className="absolute right-4 top-16 w-64 bg-surface-900 border border-surface-700 rounded-xl shadow-2xl p-4 z-20 space-y-3 animate-fade-in">
              ...
              {NODE_SHAPES.map((s) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedNode.shape === s.value ? 'bg-brand-500/20 text-brand-400' : 'bg-surface-800 text-surface-400 hover:text-white'
                  )}
                >
                  {s.label}
                </button>
          ...
          <div className="flex gap-2 pt-2 border-t border-surface-800">
            ...
            <button
              ...
              className={cn('px-3 py-1.5 rounded-lg text-xs transition-colors', selectedNode.is_locked ? 'bg-yellow-500/20 text-yellow-400' : 'bg-surface-800 text-surface-400 hover:text-white')}
            >
              ...
              <div className="flex gap-1 mt-1">
                {LINE_STYLES.map((s) => (
                  <button
                    ...
                    className={cn(
                      'px-2 py-1 text-xs rounded transition-colors',
                      selectedEdge.line_style === s.value ? 'bg-brand-500/20 text-brand-400' : 'bg-surface-800 text-surface-400 hover:text-white'
                    )}
                  >
                    {s.label}
                  </button>
              ...
              <div className="flex gap-1 mt-1">
                {ARROW_TYPES.map((a) => (
                  <button
                    ...
                    className={cn(
                      'px-2 py-1 text-xs rounded transition-colors',
                      selectedEdge.arrow_type === a.value ? 'bg-brand-500/20 text-brand-400' : 'bg-surface-800 text-surface-400 hover:text-white'
                    )}
                  >
                    {a.label}
                  </button>
        ...
        <div className="space-y-3">
          ...
          {characters.length === 0 ? (
            <p className="text-sm text-surface-500 py-4 text-center">
              ...
              <Link href={`/projects/${params.id}/characters`} className="text-brand-400 hover:underline">
