...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const { data: post } = await supabase
    .from('blog_posts')
    .select('title, excerpt, cover_image, tags, published_at, author:profiles!author_id(display_name, full_name)')
    .eq('slug', params.slug)
    .eq('status', 'published')
    .single();
  ...
  const authorName = (post.author as any)?.full_name || (post.author as any)?.display_name || 'Screenplay Studio';
  ...
  return {
    ...
    twitter: {
      ...
      title,
      description,
      images: post.cover_image ? [post.cover_image] : ['/api/og?title=' + encodeURIComponent(title)],
    },
  };
}
