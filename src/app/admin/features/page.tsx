    ...
    <div className="min-h-screen bg-surface-950">
            ...
            <div className="space-y-1.5">
              {items.map((flag) => (
                <div key={flag.id} className="rounded-xl border border-surface-800 bg-surface-900/50 px-4 py-3 flex items-center gap-4 group hover:border-surface-700 transition-colors">
                  ...
                  {/* Delete */}
                  <button onClick={() => deleteFlag(flag.id)} className="opacity-0 group-hover:opacity-100 p-1.5 text-surface-600 hover:text-red-400 transition-opacity">
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowAdd(false)}>
          <div className="bg-surface-900 border border-surface-700 rounded-xl w-full max-w-lg mx-4 p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
