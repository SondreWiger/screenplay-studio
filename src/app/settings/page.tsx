...
function PreMiDCard() {
  ...
  const [enabled,     setEnabled]     = useState(() => read('premid_enabled', true));
  const [showProject, setShowProject] = useState(() => read('premid_show_project', false));
  const [showTool,    setShowTool]    = useState(() => read('premid_show_tool', true));

  const persist = (key: string, value: boolean) => {
    try { localStorage.setItem(key, String(value)); } catch {}
    // Dispatch storage event so the hook in the layout picks it up immediately
  ...
  const toggles = [
    {
      ...
      desc: 'Shows "Playing Screenplay Studio" in your Discord status while you work.',
      value: enabled,
      setter: setEnabled,
    },
    {
      key: 'premid_show_project',
      label: 'Show project name',
      desc: 'Lets Discord contacts see which project you are working on. Off by default.',
      value: showProject,
      setter: setShowProject,
      disabled: !enabled,
    },
    {
      key: 'premid_show_tool',
      label: 'Show current tool',
      desc: 'Shows which tool you have open — "Script Editor", "Corkboard", etc.',
      value: showTool,
      setter: setShowTool,
      disabled: !enabled,
    },
    ...
    <Card className="p-6">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h2 className="text-lg font-semibold text-white flex items-center gap-2">
            {/* Discord logo mark */}
            <svg className="w-5 h-5" style={{ color: '#5865F2' }} viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="space-y-3">
        {toggles.map((t) => (
          <button
            key={t.key}
            onClick={() => toggle(t.key, t.value, t.setter)}
            disabled={t.disabled}
            className={`w-full flex items-center justify-between gap-4 p-3 rounded-lg border transition-colors text-left ${
              t.disabled
                ? 'border-surface-800 opacity-40 cursor-not-allowed'
          ...
          >
            ...
            <div
              className={`w-10 h-[22px] rounded-full shrink-0 relative transition-colors ${
                t.value && !t.disabled ? 'bg-[#5865F2]' : 'bg-surface-700'
              }`}
            >
              <div
                className={`absolute top-[3px] w-4 h-4 rounded-full bg-white shadow transition-transform ${
                  t.value && !t.disabled ? 'left-[22px]' : 'left-[3px]'
                }`}
              />
            </div>
          </button>
...
function TranslationsSettingsTab() {
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <Card className="p-6">
        <h2 className="text-lg font-semibold text-white mb-2">{t('settings.language')}</h2>
        <p className="text-sm text-surface-400 mb-6">{t('settings.language_desc')}</p>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h2 className="text-lg font-semibold text-white">{t('settings.add_language')}</h2>
            <p className="text-sm text-surface-400">{t('settings.add_language_desc')}</p>
          </div>
    ...
    <div className="space-y-6">
        ...
        {tab === 'profile' && (
          <div className="space-y-6">
            ...
            {/* Social Links */}
            <Card className="p-6">
              <h2 className="text-lg font-semibold text-white mb-2">{t('settings.social_links')}</h2>
              <p className="text-sm text-surface-400 mb-6">{t('settings.social_desc')}</p>
              <div className="space-y-3">
                {[
            ...
            <div className="flex items-center gap-3">
              ...
              {saved && <span className="text-sm text-green-400">{t('settings.changes_saved')}</span>}
            </div>
          </div>
        )}
        ...
        {tab === 'preferences' && (
          <><div className="space-y-6">
            ...
            <Card className="p-6">
              <h2 className="text-lg font-semibold text-white mb-2">{t('settings.feature_visibility')}</h2>
            ...
            <div className="flex items-center gap-3">
              <Button onClick={savePreferences} loading={saving}>
                {saved ? t('settings.saved') : t('settings.save_preferences')}
              </Button>
              {saved && <span className="text-sm text-green-400">{t('settings.changes_saved')}</span>}
            </div>
          </div>
